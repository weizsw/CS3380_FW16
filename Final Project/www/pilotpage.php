...
<html>
  <head>
    <meta charset="utf-8">
    <title>Mizzou Airlines Pilot</title>
    ...
    <link rel="stylesheet" type="text/css" href="Click_here/css/pilotpage.css">
  ...
  <body>
    ...
    <div><h1>Welcome Back Pilot! Here's Your Flights</h1> </div>
  ...
  session_start();
          ...
          $username=$_SESSION['login'];
           ...
           $link = mysqli_connect('localhost','CS3380GRP15','ee56fe1','CS3380GRP15') or die ("Connection error on Line 51: ". mysqli_connect_error());
            $info = "SELECT * FROM Pilot WHERE username  = '$username';";
            ...
            $result = mysqli_query($link,$info) or die("Query error on line 59: ".mysqli_error($link));
              ...
              while($row = mysqli_fetch_array($result, MYSQLI_NUM)){
                ...
                echo"<form method='POST' action='editpilot.php'>";
				...
				echo"<input type='hidden' name='pilot_status' value='$row[2]'>";
  ...
  <?php
            ...
            $eqp = "SELECT Equipment.plane_id, Equipment.seating_capacity, Equipment.num_pilots, Equipment.num_attendants, Equipment.plane_type, Equipment.serial_num FROM Equipment INNER JOIN PilotEquipment ON Equipment.plane_id=PilotEquipment.plane_id WHERE PilotEquipment.username='$username'";

            $result = mysqli_query($link,$eqp) or die("Query error on line 59: ".mysqli_error($link));
              echo "<br><br>";
              echo "<div class='row'>";
              echo "<h2>Your Certified Equipment</h2>";
              echo"<table>";
              while($fieldinfo = mysqli_fetch_field($result)){
                echo "<th>".$fieldinfo->name. "</th>";
              }
